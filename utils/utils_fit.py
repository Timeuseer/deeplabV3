...
def get_train_step_fn(strategy):
    ...
    if strategy == None:
        ...
        def distributed_train_step(images, labels, net, optimizer, loss, metrics):
            per_replica_losses, per_replica_score = strategy.run(train_step,
                                                                 args=(images, labels, net, optimizer, loss, metrics))
            return strategy.reduce(tf.distribute.ReduceOp.MEAN, per_replica_losses, axis=None), strategy.reduce(
                tf.distribute.ReduceOp.MEAN, per_replica_score, axis=None)
...
def get_val_step_fn(strategy):
    ...
    if strategy == None:
        ...
        def distributed_val_step(images, labels, net, optimizer, loss, metrics):
            per_replica_losses, per_replica_score = strategy.run(val_step,
                                                                 args=(images, labels, net, optimizer, loss, metrics))
            return strategy.reduce(tf.distribute.ReduceOp.MEAN, per_replica_losses, axis=None), strategy.reduce(
                tf.distribute.ReduceOp.MEAN, per_replica_score, axis=None)
...
def fit_one_epoch(net, loss, loss_history, eval_callback, optimizer, epoch, epoch_step, epoch_step_val, gen, gen_val,
                  Epoch, metrics, save_period, save_dir, strategy):
    ...
    if (epoch + 1) % save_period == 0 or epoch + 1 == Epoch:
        net.save_weights(os.path.join(save_dir, 'ep%03d-loss%.3f-val_loss%.3f.h5' % (
