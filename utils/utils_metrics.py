...
def compute_mIoU(gt_dir, pred_dir, png_name_list, num_classes, name_classes=None):
    ...
    for ind in range(len(gt_imgs)):
        ...
        if len(label.flatten()) != len(pred.flatten()):
            print(
                f'Skipping: len(gt) = {len(label.flatten()):d}, len(pred) = {len(pred.flatten()):d}, {gt_imgs[ind]:s}, {pred_imgs[ind]:s}')
            continue

        '''
        对一张图片计算21x21的hist矩阵，再累加
        '''
        hist += fast_hist(label.flatten(), pred.flatten(), num_classes)
        # 每计算10张就输出一下目前已计算的图片中所有类别泡脚的mIOU值
        if name_classes is not None and ind > 0 and ind % 10 == 0:
            print(
                f'{ind:d} / {len(gt_imgs):d}: mIou-{100 * np.nanmean(per_class_iu(hist)):0.2f}%; mPA-{100 * np.nanmean(per_class_PA_Recall(hist)):0.2f}%; Accuracy-{100 * per_Accuracy(hist):0.2f}%'
            )
    ...
    '''
    在所有验证集图像上求所有类别泡脚的mIoU值，计算时忽略空值
    '''
